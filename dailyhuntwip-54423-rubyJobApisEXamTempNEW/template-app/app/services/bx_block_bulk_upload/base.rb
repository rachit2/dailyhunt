...
module BxBlockBulkUpload
  ...
  module Base
    ...
    def add_errors(msg, name, sn, object_name, identifier)
      @error_tracker.add_errors("In #{@sheet_name}: Their was error occurred in saving #{object_name} with #{identifier} '#{name}' in row with sn: '#{sn}', errors: #{msg}")
    end
