  ...
  class LiveStreamService
    class << self
      ...
      def create_content(row, content_params, parent_error_tracker=nil)
        ...
        if content_exist
          if content_exist.update(live_stream_params(row, content_params))
      ...
      def live_stream_params(row, content_params)
        content_params.merge(publish_date: row[:publish_date], feature_article: row[:feature_article], feature_video: row[:feature_video], is_popular: row[:is_popular], is_trending: row[:is_trending], contentable_attributes: {headline: row[:heading], description: row[:description], url: row[:url] })
      end
