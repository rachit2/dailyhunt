      ...
      def import_data
        ...
        @sheet_content.each do |sheet_content|
          ...
          if sheet_content[:parent_name].present?
            parent = BxBlockCategories::SubCategory.find_by('lower(name) = ?', strip_downcase(sheet_content[:parent_name]))
            add_errors("can't find parent with name '#{sheet_content[:parent_name]}'", sheet_content[:name], sheet_content[:sn], 'sub category', 'name') unless parent.present?
          end
          if sheet_content[:category_name].present?
            category_new = find_category(sheet_content[:category_name])
            unless category_new.present?
              add_errors("can't find category with name '#{sheet_content[:parent_name]}'", sheet_content[:name], sheet_content[:sn], 'sub category', 'name')
            else
              sub_category = create_sub_category(sheet_content[:name], category_new, parent)
              if sub_category.errors.present?
                add_errors(sub_category.errors.full_messages.to_sentence, sheet_content[:name], sheet_content[:sn], 'sub category', 'name')
              end
            end
          else
            sub_category = create_sub_category(sheet_content[:name], category_new, parent)
