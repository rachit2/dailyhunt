module BxBlockLanguageoptions
  class TranslationDataImportService
    def store_data(xlsx)
      ...
      arr[1..].each do |arr2|
        return {success: false, error: "key not present in translations sheet"} unless arr2[:key].present?
        return {success: false, error: "key -#{ arr2[:key] } serial number not present in translations sheet"} unless arr2[:sn].present?
        ...
        I18n.available_locales.each do |locale|
          if application_message.id.present?
            ...
            unless translation.save
              return {success: false, error: "Some error occured in Application Message data in row- #{arr2[:sn]} -> #{translation.errors.full_messages} "}
            end
          else
            application_message.translations_attributes=  [{message: arr2[locale], locale: locale.to_s}]
          end
          unless application_message.save
            return {success: false, error: "Some error occured in Application Message data in row- #{arr2[:sn]} -> #{application_message.errors.full_messages} "}
          end
        end
      end
      return {success: true}
